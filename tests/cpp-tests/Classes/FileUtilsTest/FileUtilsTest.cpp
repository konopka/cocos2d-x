...
#include <stdio.h>                      // for fclose, fopen, FILE, fputs, etc
...
#include "FileUtilsTest/../BaseTest.h"  // for BaseTest
#include "FileUtilsTest/../testBasic.h"  // for CL
...
void TestResolutionDirectories::onEnter()
{
    ...
    for( int i=1; i<7; i++) {
        auto filename = __String::createWithFormat("test%d.txt", i);
...
void TestSearchPath::onEnter()
{
    ...
    for( int i=1; i<3; i++) {
        auto filename = __String::createWithFormat("file%d.txt", i);
        ret = sharedFileUtils->fullPathForFilename(filename->getCString());
        log("%s -> %s", filename->getCString(), ret.c_str());
    }
